    ...
    <div class="album">
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <el-tag
                        v-if="scope.row.classifyId == 1"
                        type="success"
                        close-transition>电影
                    </el-tag>
                    <el-tag
                        v-if="scope.row.classifyId == 2"
                        type="info"
                        close-transition>电视剧
                    </el-tag>
                    <el-tag
                        v-if="scope.row.classifyId == 3"
                        type="warning"
                        close-transition>动漫
                    </el-tag>
                    <el-tag
                        v-if="scope.row.classifyId == 4"
                        type="danger"
                        close-transition>小说
                    </el-tag>
                    <el-tag
        ...
        <el-form ref="addFormRef" label-position="right" :rules="rulesEditUser" :model="editUser" label-width="100px" v-if="editUser != null">
            ...
            <el-form-item label="专辑Id">
                <el-input v-model="editUser.albumId" :disabled="true"></el-input>
            </el-form-item>

            <el-form-item label="分类">
                <el-input v-bind:value="classFillter(editUser.classifyId)" :disabled="true"></el-input>
            </el-form-item>

            <el-form-item prop="albumName" label="专辑名称">
                <el-input v-model="editUser.albumName"></el-input>
            </el-form-item>

            <el-form-item prop="albumDetails" label="简介">
                <el-input
                    type="textarea"
                    :autosize="{ minRows: 2, maxRows: 4}"
                    placeholder="请输入内容"
                    v-model="editUser.albumDetails">
                </el-input>
            </el-form-item>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            rulesEditUser: {
                ...
                albumDetails: [
                { required: true, message: '专辑简介不能为空', trigger: 'blur' }
                ],
            }
        }
    },
    ...
    methods:{
        ...
        // 打开批量删除确认对话框
        open2() {
            this.$confirm('此操作将永久删除选中专辑和专辑下的所有句子, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
            }).then(() => {
        ...
        handleDelete:function(index,row){
            this.loading = true;
            console.log(row);
            this.$http.post("/api/admindelalbum",row,{
                headers: {
        ...
        handleDeleteSome:function(){
            this.loading = true;
            this.$http.post("/api/admindelsomealbum",this.multipleSelection,{
                headers: {
                    'Content-Type':'application/json;charset=UTF-8'
                }
        ...
        onEditUser:function(){
            ...
            if (this.editUser.userPicture != '') {
                this.$http.post("/api/adminupdataalbum", this.editUser, {
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8'
                    }
        ...
        handleEdit:function(index,row){
            ...
            // 给头像赋值
            this.fileList = [{name: this.editUser.albumPicture, url: this.$global.globalPictureUrl + this.editUser.albumPicture}];
        ...
        getUserList:function(){
            ...
            this.$http.post("/api/admingetalbums",this.pageReqinfo,{
            headers: {
                'Content-Type':'application/json;charset=UTF-8'
            }
            }).then(response => {
        ...
        handleSuccess(res, file) {
            ...
            if (file.response.success) {
                ...
                this.editUser.albumPicture = file.response.message; //将返回的文件储存路径赋值picture字段
        ...
        handleRemove(file, fileList) {
            ...
            this.$confirm('此操作将删除该图片, 是否继续?', '提示', {
                    ...
                    // 点确定来到了这
                    this.$http.post("/api/delpicture",this.editUser,{
                            headers: {
                                'Content-Type':'application/json;charset=UTF-8'
                            }
                        }).then(response => {
